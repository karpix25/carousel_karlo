...
#### `REDIS_URL`
- **Purpose**: Connection string for the Redis instance that backs the async queue.
- **Default**: `redis://127.0.0.1:6379/0`
- **Recommendation**: Point to a local Redis for development or a managed Redis for production.

#### `RQ_QUEUE_NAME`
- **Purpose**: Name of the Redis Queue (RQ) that accepts jobs.
- **Default**: `karpix-tasks`
- **Recommendation**: Use unique names per environment (e.g., `karpix-dev`, `karpix-prod`).

#### `RQ_DEFAULT_TIMEOUT`
- **Purpose**: Maximum execution time (seconds) for a queued job before RQ marks it as timed out.
- **Default**: `3600`
- **Recommendation**: Increase for long-running media operations.
...
- Redis must be running and at least one RQ worker (`python rq_worker.py`) must be active to drain queued jobs.

### Running the Queue Locally

1. Start Redis (`brew services start redis`, `redis-server`, or `docker run -p 6379:6379 redis`).
2. Launch the API server as usual (`python app.py` or `gunicorn app:app`).
3. In a separate terminal, start an RQ worker: `python rq_worker.py`.
4. Fire multiple requests; routes with `webhook_url` will enqueue jobs and the worker will process them asynchronously, logging progress under `LOCAL_STORAGE_PATH/jobs`.
