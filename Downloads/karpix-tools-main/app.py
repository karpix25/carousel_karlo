...
from config import REDIS_URL, RQ_QUEUE_NAME, RQ_DEFAULT_TIMEOUT
from services.rq_worker import run_queued_task
...
def create_app():
    ...
    rq_queue = RQQueue(RQ_QUEUE_NAME, connection=redis_conn, default_timeout=RQ_DEFAULT_TIMEOUT)
    queue_id = rq_queue.name
        ...
        def decorator(f):
            def wrapper(*args, **kwargs):
                ...
                if os.environ.get("GCP_JOB_NAME") and data.get("webhook_url") and not disable_cloud:
                    ...
                    current_queue_length = rq_queue.count()
                    if MAX_QUEUE_LENGTH > 0 and current_queue_length >= MAX_QUEUE_LENGTH:
                        error_response = {
                            "code": 429,
                            "id": data.get("id"),
                            "job_id": job_id,
                    ...
                    rq_queue.enqueue(
                        ...
                        args,
                        kwargs,
                        job_timeout=RQ_DEFAULT_TIMEOUT,
                        job_id=job_id
                    )
